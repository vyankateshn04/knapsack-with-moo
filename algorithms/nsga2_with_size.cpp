...
void calculate_objectives(Individual& individual, const vector<Transaction>& all_transactions, double mx_gas_units, double mx_size_bytes) {
    individual.total_gas_units = 0.0;
    individual.total_size_bytes = 0.0;
    ...
    for (int tx_id : individual.selected_idx) {
        ...
        individual.total_size_bytes += all_transactions[tx_id].rlp_size_bytes;
    }
    // Store the negative tx fee for minimization
    individual.negative_total_tx_fee = -total_tx_fee;

    // Sum of raw excess over gas and size limits.
    individual.constraint_violation =
        max(0.0, individual.total_gas_units - mx_gas_units) +
        max(0.0, individual.total_size_bytes - mx_size_bytes);
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 4) {
        cerr << "Usage: ./nsga2_with_size mempool.csv block_gas_limit block_size_limit\n";
        return 1;
    }
    ...
    double mx_size_bytes = stod(argv[3]);
    ...
    while (getline(file, line)) {
        ...
        double rlp_size_bytes = stod(columns.back()); // rlp_size_bytes

        // cout << gas_used << " , fee: " << fee_paid << endl; 
        // Store fee as double (NSGA uses double internally)
        all_transactions.push_back({idx, gas_used, (double)fee_paid, rlp_size_bytes});
        idx++;
    }
    ...
    for (auto& ind : population) {
        ...
        calculate_objectives(ind, all_transactions, mx_gas_units, mx_size_bytes);
    }
    ...
    for (int gen = 0; gen < generations; ++gen) {
        ...
        while (offspring.size() < population_size) {
            ...
            calculate_objectives(children.first, all_transactions, mx_gas_units, mx_size_bytes);
            calculate_objectives(children.second, all_transactions, mx_gas_units, mx_size_bytes);
    ...
    cout << "size: " << best_solution.selected_idx.size() << endl;

    cout << "fee: " << -best_solution.negative_total_tx_fee
         << "\ngas used:" << best_solution.total_gas_units
         << "\nsize used:" << best_solution.total_size_bytes << endl;
    return 0;
}
